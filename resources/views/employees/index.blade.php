<x-layout>
    <div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 mt-4 sm:mt-8">
        ...
        <div class="flex flex-col md:flex-row flex-wrap items-start md:items-center md:justify-between mb-8 gap-3">
            <form action="{{ route('employees.index') }}" method="GET" class="flex flex-row items-center gap-3 w-full md:w-auto">
                <div class="relative flex-1 md:w-64">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <svg class="h-4 w-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                    </div>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search employees..." class="form-input pl-10">
                </div>

                <label class="btn btn-secondary cursor-pointer shrink-0 h-[38px] !px-3">
                    <input type="checkbox" name="show_left_employees" value="1" onchange="this.form.submit()" {{ $showLeftEmployees ? 'checked' : '' }} class="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500 transition-colors">
            ...
            <div class="flex flex-row flex-wrap items-center gap-3 w-full md:w-auto">
                <a href="{{ route('employees.export.pdf', ['show_left_employees' => request('show_left_employees')]) }}" class="btn btn-secondary shrink-0 h-[38px]">
                    <svg class="mr-2 -ml-1 w-4 h-4 text-rose-500" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14v-4H8l4-4 4 4h-3v4h-2zm-3.5 2a1.5 1.5 0 110-3 1.5 1.5 0 010 3zm9 0a1.5 1.5 0 110-3 1.5 1.5 0 010 3z" /></svg>
                    Export PDF
                </a>

                @if (Auth::check() && Auth::user()->is_admin)
                    <a href="{{ route('admin.employees.import') }}" class="btn btn-secondary shrink-0 h-[38px]">
                        <svg class="mr-2 -ml-1 w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
                        Bulk upload
                    </a>
                @endif
                           ...
                           <tr class="table-row group {{ !$emp->is_active ? 'bg-slate-50/50 opacity-75' : '' }}">
                                ...
                                <td class="table-td">
                                    <div class="flex items-center">
                                        <div class="h-8 w-8 rounded-full {{ $emp->is_active ? 'bg-blue-100 text-blue-700' : 'bg-slate-200 text-slate-500' }} flex items-center justify-center font-bold text-xs mr-3">
                                            {{ substr($emp->first_name, 0, 1) }}{{ substr($emp->last_name, 0, 1) }}
                                        </div>
                                        <div class="text-sm font-medium text-slate-900">
                                            {{ $emp->fullName }}
                                            @if (!$emp->is_active)
                                                <span class="ml-2 inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-rose-100 text-rose-800">Left</span>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                                <td class="table-td">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold border {{ $emp->role_badge_classes }}">
                                        {{ $emp->role_display }}
                                    </span>
                                </td>
                                <td class="table-td text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        @if ($emp->is_active && $emp->role === 'ArticleTrainee')
                                            <button onclick="openUpgradeModal('{{ $emp->id }}', '{{ $emp->fullName }}')" class="action-icon icon-emerald" title="Upgrade to Staff">
                                        ...
                                        @if ($emp->is_active && Auth::user()->employee_id !== $emp->id)
                                            <a href="{{ route('employees.mark-left', $emp) }}" class="action-icon icon-amber" title="Offboard Employee">
                                        ...
                                        <a href="{{ route('employees.edit', $emp) }}" class="action-icon icon-blue" title="Edit">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center">
                                    ...
                                    <p class="mt-1 text-sm text-slate-500">Adjust your search or get started by creating a new record.</p>
                                    <div class="mt-6">
                                        <a href="{{ route('employees.create') }}" class="btn btn-primary bg-blue-50 text-blue-600 hover:bg-blue-100 !border-transparent shadow-sm">
                ...
                <form id="upgradeForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="new_designation" value="Audit Assistant" required class="form-input">
                        </div>
                        <div>
                            <label class="form-label">Completion date</label>
                            <input type="date" name="completion_date" value="{{ date('Y-m-d') }}" required class="form-input">
                        </div>
                        <div>
                            <label class="form-label">Completion certificate (PDF)</label>
                            <input type="file" name="certificate" accept=".pdf" class="form-file-input !bg-blue-50 !text-blue-700 hover:!bg-blue-100">
                        </div>
                    </div>

                    <div class="mt-8 flex justify-end space-x-3">
                        <button type="button" onclick="closeUpgradeModal()" class="px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 rounded-lg transition-colors">Cancel</button>
                        <button type="submit" class="btn btn-primary !bg-emerald-600 hover:!bg-emerald-700">Confirm upgrade</button>
                    </div>
                </form>
            </div>
        </div>
