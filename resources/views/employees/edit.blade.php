<x-layout>
    <div class="max-w-4xl mx-auto p-6 bg-white shadow-sm rounded-lg mt-8">
        <h2 class="text-2xl font-semibold text-gray-800 mb-6">Edit Employee: {{ $employee->first_name }} {{ $employee->last_name }}</h2>
        ...
        <form action="{{ route('employees.update', $employee) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            @method('PUT') <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="emp_code" value="{{ old('emp_code', $employee->emp_code) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                <div>
                    ...
                    <input type="text" name="first_name" value="{{ old('first_name', $employee->first_name) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('first_name') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700">Last Name *</label>
                    <input type="text" name="last_name" value="{{ old('last_name', $employee->last_name) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            @if($employee->articleship_deed_path)
                                <div class="text-sm text-green-600 mb-2 font-medium">✓ Document currently uploaded</div>
                            @endif
                            <input type="file" name="articleship_deed_pdf" accept=".pdf" class="mt-1 block w-full text-sm text-gray-500">
                            <span class="text-xs text-gray-500">Upload a new file to replace the existing one.</span>
                            @error('articleship_deed_pdf') <br><span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200">
                ...
                <button type="submit" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md shadow-sm font-medium">Update Employee</button>
            </div>
        </form>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            toggleTraineeSection(); // Run on load to maintain state
        });
    </script>
</x-layout>