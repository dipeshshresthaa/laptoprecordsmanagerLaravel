<x-layout>
    <div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 mt-0">
        ...
        <div class="mb-6">
            ...
            <p class="mt-1 text-sm text-slate-500">View partner directories, trainee registers, and principal
                allocations.</p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <form action="{{ route('reports.index') }}" method="GET"
                ...
                <div class="flex items-center shrink-0">
                    <select name="status" onchange="this.form.submit()"
                        ...
                        <option value="left" {{ $status === 'left' ? 'selected' : '' }}>🔴 Left / Inactive employees
                        </option>
                    </select>
                </div>
            ...
            <div
                class="border-b border-slate-200 bg-white px-4 pt-4 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
                <nav class="-mb-px flex space-x-6 overflow-x-auto pb-1" aria-label="Tabs">
                    <button onclick="switchTab(0)" id="tab-btn-0"
                        class="tab-button whitespace-nowrap py-3 px-1 border-b-2 font-bold text-sm border-blue-600 text-blue-600">
                        👨‍💼 Partners list ({{ $partners->count() }})
                    </button>
                    <button onclick="switchTab(1)" id="tab-btn-1"
                        class="tab-button whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300">
                ...
                <div class="pb-3 shrink-0">
                    <a href="{{ route('reports.export_comprehensive', ['search' => $search]) }}" target="_blank"
                        ...
                        <svg class="mr-1.5 h-4 w-4 text-rose-500" fill="currentColor" viewBox="0 0 24 24">
                            <path
                    ...
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead>
                            <tr>
                                ...
                                <th class="text-left text-xs font-semibold text-slate-500 uppercase pb-3">Department
                                </th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            ...
                            @empty
                                <tr>
                                    <td colspan="2" class="py-4 text-center text-sm text-slate-500">No partners found
            ...
            <div id="tab-content-1" class="tab-content hidden p-6">
                <div class="overflow-x-auto w-full">
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead>
                            <tr>
                                <th class="text-left text-xs font-semibold text-slate-500 uppercase pb-3">Trainee name
                                </th>
                                <th class="text-left text-xs font-semibold text-slate-500 uppercase pb-3">Assigned
                                    principal</th>
                                <th class="text-left text-xs font-semibold text-slate-500 uppercase pb-3">Department
                                </th>
                            </tr>
                            ...
                            @forelse($trainees as $trainee)
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="py-3 text-sm text-slate-600">
                                        {{ $trainee->principal->full_name ?? 'Unassigned' }}</td>
                                    <td class="py-3 text-sm text-slate-600">{{ $trainee->department }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="py-4 text-center text-sm text-slate-500">No trainees found
                                        matching criteria.</td>
                                </tr>
                            @endforelse
                        </tbody>
            ...
            <div id="tab-content-2" class="tab-content hidden p-6">
                <div class="overflow-x-auto w-full">
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead>
                            <tr>
                                <th class="text-left text-xs font-semibold text-slate-500 uppercase pb-3">Principal name
                                </th>
                                <th class="text-left text-xs font-semibold text-slate-500 uppercase pb-3">Assigned
                                    trainees</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse($principalStats as $stat)
                                <tr class="hover:bg-slate-50">
                                    <td class="py-3 text-sm font-medium text-slate-900">
                                        {{ $stat->principal->full_name ?? 'Unknown' }}</td>
                                    <td class="py-3 text-sm text-slate-600">
                                        <span
                                            class="bg-blue-100 text-blue-800 text-xs font-bold px-2.5 py-0.5 rounded-full">{{ $stat->trainee_count }}</span>
                                    </td>
                                    <td class="py-3 text-right text-sm">
                                        <button
                                            onclick="openDetailsModal('{{ route('reports.api.trainees', ['id' => $stat->principal_id, 'status' => $status]) }}', '{{ addslashes(($stat->principal->first_name ?? '') . ' ' . ($stat->principal->last_name ?? '')) }}')"
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="py-4 text-center text-sm text-slate-500">No principals
                                        found matching criteria.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div id="tab-content-3" class="tab-content hidden p-6">
                <div class="overflow-x-auto w-full">
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead>
                            <tr>
                                <th class="text-left text-xs font-semibold text-slate-500 uppercase pb-3">Employee name
                                </th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse($totalStaff as $staff)
                                <tr class="hover:bg-slate-50 transition-colors">
                                    <td class="py-3 text-sm font-bold text-slate-900">
                                        {{ $staff->full_name }}
                                        <div class="text-xs text-slate-500 font-normal mt-0.5">Assigned to:
                                            {{ $staff->principal ? $staff->principal->full_name : 'None' }}</div>
                                    </td>
                                    <td class="py-3 text-sm font-medium text-slate-600">{{ $staff->role_display }}
                                    </td>
                                    <td class="py-3 text-sm text-slate-600">{{ $staff->department ?? '-' }}</td>
                                    <td class="py-3 text-sm">
                                        @if ($staff->is_active)
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-100 text-emerald-800 border border-emerald-200">
                                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 mr-1.5"></span>
                                                Active
                                            </span>
                                        @else
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-rose-100 text-rose-800 border border-rose-200">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="py-4 text-center text-sm text-slate-500">No employees
                                        found matching criteria.</td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    <div id="detailsModal" class="relative z-50 hidden" aria-labelledby="modal-title" role="dialog"
        aria-modal="true">
        <div class="fixed inset-0 bg-slate-900 bg-opacity-50 transition-opacity"></div>
        <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                <div
                    class="relative transform overflow-hidden rounded-xl bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl border border-slate-200">
                    <div class="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-center">
                        <h3 class="text-lg font-bold leading-6 text-slate-900" id="modal-title">Trainees under
                            principal</h3>
                        ...
                        <table id="modal-table" class="min-w-full divide-y divide-slate-200 hidden">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-left text-xs font-semibold text-slate-500 uppercase pb-2">Joining
                                        date</th>
                                </tr>
                            </thead>
                    ...
                    <div class="bg-slate-50 px-6 py-4 border-t border-slate-200 flex justify-end">
                        <button type="button" onclick="closeDetailsModal()"
                            class="inline-flex justify-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500">Close</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        function openDetailsModal(url, principalName) {
            ...
            fetch(url)
                ...
                .then(data => {
                    if (data.length === 0) {
                        tbody.innerHTML =
                            '<tr><td colspan="2" class="py-4 text-sm text-slate-500 text-center">No trainees found.</td></tr>';
                    } else {
                        data.forEach(trainee => {
                            tbody.innerHTML += `
                                <tr class="hover:bg-slate-50">
                                    <td class="py-3 text-sm font-medium text-slate-900">${trainee.first_name} ${trainee.last_name}</td>
                                    <td class="py-3 text-sm text-slate-600">    ${formatDate(trainee.joining_date)}</td>
                                </tr>`;
                        });
                    }
